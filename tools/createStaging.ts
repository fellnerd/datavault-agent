...
export const createStagingSchema = z.object({
  entityName: z.string().describe('Name der Entity (z.B. "product", "company")'),
  concept: z.string().describe('Business Concept/Source (z.B. "werkportal", "jira", "adventureworks")'),
  externalTable: z.string().describe('External Table Name (z.B. "ext_<concept>_<entity>")'),
...
export async function createStaging(input: CreateStagingInput): Promise<string> {
  const { entityName, concept, externalTable, businessKeyColumns, payloadColumns, foreignKeys = [] } = input;
  
  const stagingName = `${concept}_${entityName}`;
...
Supports composite business keys (multiple columns).`,
    input_schema: {
      ...
      properties: {
        entityName: {
          type: 'string',
          description: 'Entity name without prefix (e.g., "product", "company")',
        },
        concept: {
          type: 'string',
          description: 'Business concept/source system (e.g., "werkportal", "jira", "adventureworks")',
        },
        externalTable: {
          type: 'string',
          description: 'External table name (e.g., "ext_<concept>_<entity>")',
        },
        businessKeyColumns: {
          type: 'array',
          items: { type: 'string' },
      ...
      required: ['entityName', 'concept', 'externalTable', 'businessKeyColumns', 'payloadColumns'],
    },
  };
